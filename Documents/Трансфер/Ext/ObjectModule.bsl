...
Процедура ПеревестиИгрокаВКлубПокупатель()
	...
	Если КлубПродавец = Константы.ФутбольныйКлубМенеджера.Получить() Тогда
		КлубПродавецОбъект = КлубПродавец.ПолучитьОбъект();
		Для Каждого Строка Из КлубПродавецОбъект.СтартовыйСостав Цикл
			Если Строка.Футболист = Футболист Тогда
				Строка.Футболист = Справочники.Футболисты.ПустаяСсылка();
			КонецЕсли;
		КонецЦикла; 
		КлубПродавецОбъект.Записать();
	Иначе
