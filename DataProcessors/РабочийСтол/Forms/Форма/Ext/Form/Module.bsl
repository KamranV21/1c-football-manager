...
Процедура РазыгратьИгровойДень(Команда)
	...
	Если Не ПроверитьЗаполнениеКомандыМенеджера() Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ПроверитьЗаполнениеКомандыМенеджера()

	ФутбольныйКлубОбъект = ФутбольныйКлуб.ПолучитьОбъект();
	Если Не ФутбольныйКлубОбъект.ПроверитьЗаполнение() Тогда 
		ТекстОшибки = НСтр("ru = 'В вашей команде обнаружены проблемы заполнения состава. Пожалуйста, перейдите в раздел ""Тактика"" и устраните проблемы.'");
